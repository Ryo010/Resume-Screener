...
class ResumeProcessor:
    ...
    def parse_resume_from_path(self, file_path: str) -> Dict:
        ...
        logger.info(
            f"Extracted {len(resume_text)} characters from {file_path}")
    ...
    def parse_resume_from_bytes(self, file_bytes: bytes, filename: str) -> Dict:
        ...
        Returns:
            Parsed resume as dictionary
        """
        resume_text = self.extractor.extract_text_from_bytes(
            file_bytes, filename)
        logger.info(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "screen":
        ...
        try:
            ...
            print(
                f"💼 Experience: {screened['experience_match']['score']:.1f}/10")
            print(
                f"🎓 Education: {screened['education_match']['score']:.1f}/10")
            print(f"🚀 Projects: {screened['project_match']['score']:.1f}/10")
            print(
                f"📜 Certifications: {screened['certification_match']['score']:.1f}/10"
            )
            print(
                f"🤝 Cultural Fit: {screened['cultural_fit']['score']:.1f}/10")
