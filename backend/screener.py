...
class ResumeScreeningResult(BaseModel):
    ...
    recommendation: str = Field(
        ...,
        description="Hiring recommendation: 'Must Be Interviewed', 'Should Be Interviewed', 'Could Be Interviewed', 'Potential To Be Interviewed', or 'Not To Be Interviewed'"
    )
...
class ResumeScreener:
    ...
    def screen_resume(
        ...
        SCORING SCALE:
        9-10: Must Be Interviewed, rare to find better
        7-8: Should Be Interviewed, highly qualified
        5-6: Could Be Interviewed, qualified with some gaps
        3-4: Potential To Be Interviewed, significant gaps but trainable
        0-2: Not To Be Interviewed, major misalignment
        """
        ...
        try:
            ...
            logger.info(
                f"Resume screening completed. Overall score: {screening_result.overall_score}/10")
    ...
    def _format_resume_for_screening(self, parsed_resume: Dict) -> str:
        ...
        if parsed_resume.get('external_links'):
            ...
            if links.get('linkedin'):
                link_list.append(f"LinkedIn: {links['linkedin']}")
            if links.get('github'):
                link_list.append(f"GitHub: {links['github']}")
            if links.get('portfolio'):
                link_list.append(f"Portfolio: {links['portfolio']}")
            if link_list:
                sections.append("\nPROFESSIONAL LINKS:\n" +
                                "\n".join(link_list))
        ...
        if parsed_resume.get('extracurricular_activities'):
            ...
            for activity in parsed_resume['extracurricular_activities']:
                sections.append(
                    f"- {activity.get('name', 'N/A')} ({activity.get('role', 'N/A')})")
